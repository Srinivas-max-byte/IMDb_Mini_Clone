...
async function loadMovies(searchMovie) {
  await fetch(`https://omdbapi.com/?s=${searchMovie}&page=1&apikey=38b4f10a`)
...
function displayMovieList(movies) {
  ...
  for (let idx = 0; idx < movies.length; idx++) {
    ...
    movieListItem.classList.add("search-results-item");//Style for this class is defined in CSS.

    // Hits another API where Actors working in the film are stored
    fetch(`http://www.omdbapi.com/?i=${movies[idx].imdbID}&apikey=38b4f10a`)
      .then((response) => response.json())
      .then((data) => {
        let moviePoster;
...
function loadMovieDetails(){
  ...
  searchListMovies.forEach(movie => {
      movie.addEventListener('click', () => {
          fetch(`http://www.omdbapi.com/?i=${movie.dataset.id}&apikey=38b4f10a`)
          .then((response) => response.json())
          .then((data) => {
...
function displayMovieDetails(details){
  ...
  <div id = "movie-info-desc" data-poster=${details.Poster} data-id=${details.imdbID}>
      <h1 class = "movie-title dark-color">${details.Title}</h1>
      <p class = "dark-color"><span>Year: </span>${details.Year}<span>, Rating: </span>${details.Rated}<span>, Released: </span>${details.Released}</p>
      <p class = "dark-color"><span>Genere:</span> ${details.Genre}</p>
      <p class = "dark-color"><span>Writer:</span> ${details.Writer}</p>
      <p class = "dark-color"><span>Actors:</span> ${details.Actors}</p>
      <p class = "dark-color"><span>Plot:</span> ${details.Plot}</p>
      <p class="italic mv-lang" class = "dark-color"><span class = "dark-color">Language: </span>${details.Language}</p>
      <p class = "dark-color"><span><i class="fa-solid fa-award"></i></span>&nbsp;&nbsp;  ${details.Awards}</p>
      <p class = "dark-color"><span>IMDb Rating</span>&nbsp;&nbsp;  ${details.imdbRating}</p>
      <i class="fa-regular fa-heart addToFav" onclick="fav(this)">Add to favourites</i>
  </div>
  `;
...
function showFavList(){
  ...
  if(arr == null || arr.length == 0) {
      window.alert("Favourites Movie List is Empty");
      return;
  }
